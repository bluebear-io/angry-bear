...
Thank you for your interest in contributing to care-bare! This project is open source under the Apache 2.0 license, and we welcome contributions of all kinds.

## Reporting Bugs

Use [GitHub Issues](https://github.com/Blue-Bear-Security/care-bare/issues). Please include:

- **Go version:** `go version`
- **OS:** macOS, Linux, or Windows with version
- **care-bare version:** `care-bare version`
- **Steps to reproduce**
- **Expected vs actual behavior**

## Suggesting Features

Use [GitHub Issues](https://github.com/Blue-Bear-Security/care-bare/issues) with the `enhancement` label. Describe the **use case**, not just the solution.
...
| `InstallHook()` | Installs care-bare hooks into the agent's global config file |
...
This is the primary way the community extends care-bare. Here's the complete guide:
...
func (a *MyAgentAdapter) InstallHook(projectDir string) error {
    // Add care-bare hook to the agent's GLOBAL config file
...
If your agent uses different tool names than care-bare's canonical names, create a mapping in your adapter (see `cursorToolMap` in `cursor.go` for an example):
...
2. **Fail-open on config/state errors** — never block developers due to care-bare bugs
