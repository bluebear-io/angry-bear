...
// responses, detecting skill invocations, and installing the care-bare hook
...
func resolveCareBareCommand(binaryPath string) string {
	...
	if err != nil {
		return "care-bare" // fallback to PATH-based lookup
	}
...
// InstallHook adds a care-bare PreToolUse hook to the GLOBAL Claude Code settings.
...
func (a *ClaudeAdapter) InstallHook(projectDir string) error {
	...
	// Check if care-bare hook already exists (idempotency check)
...
// contains the care-bare hook command. Used for idempotency.
