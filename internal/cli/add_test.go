// add_test.go contains integration tests for the care-bare add command.
...
func readConfigFromDir(t *testing.T, dir string) engine.Config {
	t.Helper()
	data, err := os.ReadFile(filepath.Join(dir, ".care-bare", "skill_enforcement.json"))
	if err != nil {
...
func TestAdd_CreatesRulesWithDefaults(t *testing.T) {
	...
	// Create .care-bare directory so project root resolves.
...
// TestAdd_CreatesConfigDirectory verifies that add creates the .care-bare
