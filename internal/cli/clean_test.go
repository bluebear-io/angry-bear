// clean_test.go contains integration tests for the angry-bear clean command.
...
func TestClean_HandlesEmptyStateDirectory(t *testing.T) {
	...
	// Create the .angry-bear/state/ directory but no session files.
	writeEnforcementConfig(t, dir, []engine.Rule{})
	stateDir := filepath.Join(dir, ".angry-bear", "state")
	err := os.MkdirAll(stateDir, 0o755)
	if err != nil {
...
func TestClean_MalformedGlobalConfig(t *testing.T) {
	...
	err = os.WriteFile(filepath.Join(dir, ".angry-bear", "config.json"), []byte("{bad json"), 0o644)
	if err != nil {
		t.Fatalf("failed to write config.json: %v", err)
	}
