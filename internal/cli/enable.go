// enable.go implements care-bare enable and care-bare disable commands.
// Enable installs hooks into all detected agent configs.
// Disable removes ALL care-bare hooks from all agent configs.
package cli
...
func NewEnableCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "enable",
		Short: "Install care-bare hooks into all AI agent configs",
...
func NewDisableCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "disable",
		Short: "Remove ALL care-bare hooks from AI agent configs",
		Long:  "Removes all care-bare hook entries from Claude Code and Cursor.\nEnforcement stops immediately. Rules are preserved — run 'enable' to re-activate.",
		RunE:  runDisable,
	}
}
...
func runDisable(cmd *cobra.Command, args []string) error {
	...
	if removed == 0 {
		...
		fmt.Fprintln(out, "Run 'care-bare enable' to re-activate.")
	}
	return nil
}
