...
// is blocked, a BLOCK entry is written to ~/.care-bare/events.log with
...
func TestHook_LogEventWritesBlockEntry(t *testing.T) {
	...
	// Read the events log.
	logPath := filepath.Join(dir, ".care-bare", "events.log")
...
func TestHook_LogSkillEventWritesEntry(t *testing.T) {
	...
	projectDir := filepath.Join(dir, "myproject")
	if err := os.MkdirAll(filepath.Join(projectDir, ".care-bare"), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestHook_NoStateDir(t *testing.T) {
	...
	// Delete state directory (writeEnforcementConfig creates .care-bare/ but not state/).
	stateDir := filepath.Join(dir, ".care-bare", "state")
...
func TestHook_GitRepoProjectResolveIdentity(t *testing.T) {
	...
	// Create .care-bare inside the project.
	if err := os.MkdirAll(filepath.Join(projectDir, ".care-bare"), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestHook_GitRepoProjectSkillInvocation(t *testing.T) {
	...
	stateDir := filepath.Join(projectDir, ".care-bare", "state")
	mgr := state.NewStateManager(stateDir)
...
func TestHook_EnforcementConfigUnreadablePermissions(t *testing.T) {
	...
	// Create a valid .care-bare directory with enforcement config.
	configDir := filepath.Join(dir, ".care-bare")
	if err := os.MkdirAll(configDir, 0o755); err != nil {
		t.Fatal(err)
	}
