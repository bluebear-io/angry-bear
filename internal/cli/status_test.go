// status_test.go contains integration tests for the angry-bear status command.
...
func TestStatus_DisplaysActiveSessions(t *testing.T) {
	...
	// Create .angry-bear directory and enforcement config so project root resolves.
	writeEnforcementConfig(t, dir, []engine.Rule{})
...
func TestStatus_DisplaysDiscoveredSkills(t *testing.T) {
	...
	// Create .angry-bear with config pointing to .claude/skills.
	writeEnforcementConfig(t, dir, []engine.Rule{})
...
// no .angry-bear directory gracefully, displaying empty/default state.
