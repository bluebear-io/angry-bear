...
func TestApp_ViewSettings(t *testing.T) {
	...
	if !strings.Contains(output, "angry-bear") {
		t.Error("expected angry-bear header in app view")
	}
}
...
func TestSaveGlobalConfig_GlobalLevel(t *testing.T) {
	...
	configPath := tmpHome + "/.angry-bear/config.json"
...
func TestApp_EventsUpdatedMsg(t *testing.T) {
	...
	// The reload reads from ~/.angry-bear/events.log.
